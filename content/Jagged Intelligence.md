...
체스가 대표적 예다. GPT-4가 체스에서 향상된 것이 일반 지능의 진보가 아니라, 사전훈련 믹스에 체스 데이터가 대량으로 포함되었기 때문일 수 있다. OpenAI의 누군가가 그 데이터를 추가하기로 결정했고, 거기에 능력 스파이크가 생긴 것이다. 프론티어 모델은 사전훈련 믹스, RL 환경, 벤치마크 압력의 인공물(artifact)이며 매뉴얼이 없다. 출처: `raw/sequoia-ascent-2026-karpathy-ko.md`
