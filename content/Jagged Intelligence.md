...
Jagged Intelligence는 [[LLM]]이 어떤 영역에서는 천재처럼 보이지만 바로 옆 영역에서는 상식적인 실수를 하는 들쭉날쭉한 능력 분포다.

## 핵심 요지

- 카파시의 요지는 "전통적 컴퓨터는 코드로 명시 가능한 것을 자동화했고, 현재 [[LLM]]은 검증 가능한 것을 자동화한다"는 것이다.
...
[[Sequoia Ascent 2026]]에서 카파시는 이 현상의 원인을 단순화했다. 능력 스파이크는 검증 가능성뿐만 아니라 "연구소들이 훈련, 사후 훈련, 합성 데이터 생성, 강화학습 중에 그 과제를 얼마나 강조했는지"에도 달려 있다. 대략적인 공식은:
