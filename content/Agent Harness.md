...
`raw/아이디어에서 현실 시스템까지 AI 에이전트 구축하기 - 출판형 다듬기.md`는 이 구조를 목적·범위·model·memory·tools·orchestration·evaluation이라는 더 전형적인 blueprint 언어로 다시 정리한다. 즉 agent를 만든다는 것은 단순히 LLM을 고르는 일이 아니라, 목적과 검증을 포함한 시스템 전체를 조립하는 일임을 강조한다.
