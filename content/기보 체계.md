...
- **시나리오**: 음악의 기보 체계인 **ABC Notation**을 파싱하여 정형화된 MIDI 데이터(JSON 형태)로 변환하는 [[LLM]] 에이전트 구축.
  - ABC Notation은 전통 음악 등에서 널리 쓰이는 대표적인 텍스트 기반 음악 기보 체계다. 통사적 규칙(예: `C`는 도, `D`는 레, `^`는 샵 등)과 의미적 조건(고정된 음높이와 박자)을 충족하므로 [[LLM]]이 코드를 생성하여 쉽게 파싱할 수 있다.
...
## 관련 노트
- [[어포던스]]: 사물과 유기체 간의 관계적 속성으로, 예술 기호를 해독하는 인간의 내적 도식(schemata) 및 감상 능력이 [[어포던스]]의 인식과 닮아 있다. (raw/What Makes Us Human.md)
