...
## 충돌
- **[[OpenRouter]] 비용 트레이드오프**: API 게이트웨이인 [[OpenRouter]]는 유료 요금제 사용 시 제공업체 라우팅 경로의 불투명성으로 인해, 동일한 모델임에도 선택 경로에 따라 요금 차이가 3~7배까지 날 수 있다. 대규모 트래픽 프로젝트 적용 시 단일 API 호출보다 전용 엔드포인트 연동(Groq 또는 NIM)을 직결하는 것이 비용 예측성 면에서 유리하다.
