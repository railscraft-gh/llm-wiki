...
## 한 줄 정의
완전히 기계 읽기 가능한 [[디자인 시스템]]은 AI 에이전트가 코드를 작성할 때 [[디자인 시스템]]의 Foundations(기초 규칙)와 Component specs(컴포넌트 규격)를 맥락에 맞춰 올바르게 적용하도록 설계한 계층화된 맥락 시스템(layered context system)이다.
...
6. **Agent Mappings (에이전트 매핑)**: Anthropic의 [[Claude Code]]용 플러그인 설정 및 [[Cursor]]용 MCP 서버 경로.
...
Indeed의 [[디자인 시스템]]은 구직자 및 고용주 플랫폼 전반에서 **2,000명이 넘는 R&D 디자이너와 엔지니어**가 사용하고 있다. [[디자인 시스템]] 팀은 14개 실서비스 코드베이스를 대상으로 Sourcegraph MCP 감사를 실행하여, 총 **1,697개 파일에서 6,147건의 간격 토큰 사용 사례**를 분석했다. 이를 통해 디자이너의 주관적 추측을 배제한 6개 핵심 간격 토큰과 4개 글로벌 레시피를 도출했다.
...
플러그인 구동 테스트 시 가상 화면에서 활성화된 카드(Active cards)의 왼쪽 테두리 강조선(Left-border accent)이 지속적으로 렌더링되는 문제가 발생했다. 원인을 분석한 결과, 해당 스타일은 [[디자인 시스템]]상으로 이미 수년 전에 폐기(deprecated)되었으나 사내 공식 가이드라인 문서에 권장 사항으로 잔존해 있었고, [[LLM]]은 이를 문자 그대로 신뢰하여 렌더링한 것이었다. 즉시 원본 문서를 수정하고 플러그인의 `quality.md`에 명확한 금지 규정을 보완한 결과 다음 테스트에서는 의도대로 강조선이 사라졌다.
