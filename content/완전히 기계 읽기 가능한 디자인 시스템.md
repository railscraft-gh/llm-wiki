...
플러그인 구동 테스트 시 가상 화면에서 활성화된 카드(Active cards)의 왼쪽 테두리 강조선(Left-border accent)이 지속적으로 렌더링되는 문제가 발생했다. 원인을 분석한 결과, 해당 스타일은 디자인 시스템상으로 이미 수년 전에 폐기(deprecated)되었으나 사내 공식 가이드라인 문서에 권장 사항으로 잔존해 있었고, [[LLM]]은 이를 문자 그대로 신뢰하여 렌더링한 것이었다. 즉시 원본 문서를 수정하고 플러그인의 `quality.md`에 명확한 금지 규정을 보완한 결과 다음 테스트에서는 의도대로 강조선이 사라졌다.
