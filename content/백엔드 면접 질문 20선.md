...
8. **결제 재시도 엔드포인트의 멱등성 및 결제 예외 처리**
   ...
   - **결제 미캡처 예외 처리 (Payment Never Captured Exception Handling)**: 2단계 결제 방식(Authorize 후 Capture)을 사용하는 경우, 승인(Authorize)은 성공했으나 이후 통신 오류로 인해 매입(Capture)을 실행하지 못하는 예외가 발생할 수.
