...
18. **Kafka 컨슈머의 오프셋 커밋 실패**
    ...
    - **대책**: 이를 감안해 컨슈머가 처리하는 핵심 비즈니스 로직(예: DB 저장)은 항상 [[멱등성]]이 보장되거나 중복 제거를 하도록 설계해야 한다.
...
## 출처
- Emily, "I will never walk into a backend interview without solving these 20 questions." (Medium, raw/I will never walk into a backend interview without solving these 20 questions..md)
- `raw/이 20가지 문제를 풀지 않고는 백엔드 면접에 절대 들어가지 않겠다.md`
- PracHub 백엔드 질문 아카이브 (https://prachub.com)
