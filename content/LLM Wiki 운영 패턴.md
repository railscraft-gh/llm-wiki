...
운영 단위는 세 레이어로 나뉜다. `raw/`는 웹 클리핑, 챗 로그 JSON, 원본 텍스트 등 가공되지 않은 순수한 '기억의 저장고'로서 원천 자료를 보존하며 절대 수정을 금지한다. `wiki/`는 Ingest 필터링을 통과하여 2차 가공 및 상호 연결된 지식 링크를 축적하는 '이해의 발현' 공간이다. 에이전트에게 raw 폴더 데이터의 수정 권한을 부여할 경우, 요약 및 덮어쓰기 과정에서 오리지널 정보가 왜곡되거나 훼손되는 리스크가 존재하므로 두 영역은 물리적으로 엄격히 분리되어야 한다. (출처: raw/Claude Code와 Obsidian으로 AI 세컨드 브레인 구축하기.md) 마지막으로 schema나 instruction 파일은 에이전트가 어떤 형식과 절차로 움직일지 규정하는 층이다. 이 분리는 [[Agent Native Infrastructure]]의 관점에서 중요하다. 클릭과 기억 대신 파일 구조, 문서 규칙, 로그가 에이전트의 작업 인터페이스가 되기 때문이다.

이러한 지식 저장소 아키텍처 위에서 에이전트(예: Claude Code)는 사용자가 내리는 4가지 슬래시 명령어를 통해 루프를 수행한다. (출처: raw/Claude Code와 Obsidian으로 AI 세컨드 브레인 구축하기.md)
- **/ingest**: raw 폴더 안의 어질러진 원본 파일들을 훑어 핵심 개념(엔티티)을 추출하고, 개별 위키 페이지 신설 및 유기적 링크 매핑을 수행하여 wiki 폴더로 이관한다.
- **/query**: 위키 라이브러리를 바탕으로 자연어 질의에 답하며, 답변의 신뢰도를 보장하기 위해 참조한 원천 raw 파일의 경로 및 시작된 줄(Line) 번호 등 명확한 출처를 표기한다.
- **/lint**: 깨지거나 연결이 닿지 않는 고아 페이지, Frontmatter 누락, 30일 이상 미갱신 핵심 노트를 대시보드로 경고하여 볼트 청소 및 무결성을 점검한다.
- **/log**: 다른 개념이나 파일 수정 없이 불현듯 스친 아이디어와 영감을 타임스탬프와 함께 일일 행동 일지에 빠르게 기록한다.
