...
## 상세
MoE 모델은 모든 파라미터가 동시에 일하지 않는 '희소성(Sparsity)'을 가집니다. [[TurboQuant]]-[[MLX]]는 이 특성을 활용하여 메모리에 전체 모델을 올리는 대신, 필요한 부분만 선택적으로 스트리밍하는 방식으로 하드웨어 제약을 우회합니다. 이 방식은 시스템 병목을 메모리 용량에서 디스크 대역폭(Disk I/O)으로 이동시키며, 고비용 RAM 증설 없이도 거대 모델 실행을 가능하게 하는 혁신적인 대안을 제시합니다.
