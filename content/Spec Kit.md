...
- `/speckit.constitution`: 테스트 규칙, 컴포넌트 표준, 아키텍처 규칙 등 타협 불가능한 최상위 핵심 원칙을 제정한다. 이는 항공 분야의 **10,000피트 규칙(Sterile Cockpit Rule)**처럼 개발 내내 에이전트의 불필요한 행동이나 설계 이탈을 금지하는 아키텍처 DNA 역할을 수행한다. (출처: [[집중력을 즉각적으로 향상시키는 1가지 비행 규칙]])
