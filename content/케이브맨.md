...
- 학술적 근거: MD Azizul Hakim(2026, arxiv:2604.00025) 연구에 따르면, LLM에 출력 길이 제약(Brevity constraints)을 가하면 overthinking이 억제되어 정확도가 최대 26%p 상승하며, 대형 모델이 소형 모델 대비 7.7%~15.9% 향상된 효율을 보임.
...
arxiv:2604.00025 논문에 따르면 에이전트의 생각 분량을 물리적으로 제약하는 것은 LLM이 쓸데없는 추론 루프(overthinking)에 빠져 답을 틀리는 확률을 줄이며, 이는 26%p의 놀라운 정확도 향상으로 직결된다.
