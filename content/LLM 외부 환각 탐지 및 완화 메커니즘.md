...
2. **SelfCheckGPT**: 외부 KB 없이 블랙박스 [[LLM]]에서 추출한 여러 샘플 간의 문장 일관성(BERTScore, NLI)으로 환각 탐지.
...
## 충돌
- **SFT 지식 업데이트 vs [[RAG]] 지식 주입**: 파인튜닝을 통해 모델 내부에 직접 새 사실을 새기려 하면 환각률이 올라간다. 지식 업데이트는 [[RAG]]로 컨텍스트 주입하고, 파인튜닝은 사실성 태도/정렬(Alignment)에만 한정해야 한다.
