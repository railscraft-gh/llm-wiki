...
## 한 줄 정의
개인의 머릿속에 있는 가공되지 않은 생각, 프로젝트 아이디어, 할 일 등을 AI 에이전트에게 여과 없이 쏟아내면, 에이전트가 이를 분석 및 분류하여 세컨드 브레인(Second Brain)의 적절한 위치에 마크다운(`.md`) 파일로 자동 저장하고 태스크를 추출하는 자율형 지식 수집(Capture) [[워크플로]]우입니다.
