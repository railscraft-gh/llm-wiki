...
- **연동 방식**: NocoBase가 제공하는 AI Skills, CLI, MCP(Model Context Protocol) 규격을 활용한다.
...
- **연동 방식**: 정밀한 화면 레이아웃과 데이터 처리 인터페이스는 Appsmith가 담당하고, Hermes는 기획안 및 연동 논리를 구성한다.
...
- **연동 방식**: 절차 지향적인 양식 라우팅 및 상태 전이 규칙 설계에 결합한다.
...
- **연동 방식**: Directus MCP를 기본 활용하여 에이전트가 데이터베이스 정보를 직접 읽고 수정하며, RBAC(역할 기반 권한 제어)를 작동시켜 보안을 유지한다.
...
- **연동 방식**: Airtable의 오픈소스 대체제로서, 정형 데이터 정비 및 카테고리 속성 매핑을 수행한다.
