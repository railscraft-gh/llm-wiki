...
- Cursor에서는 `/add-plugin superpowers`로 설치한다고 소개한다.
...
글에서 소개한 설치 명령은 다음과 같다.

```bash
/add-plugin superpowers
```

설치 후에는 에이전트가 대화 내용을 보고 적절한 스킬을 읽는다. 특정 스킬을 직접 쓰고 싶다면 다음 방식이 가능하다고 정리한다.
...
- 서브에이전트를 쓸 수 있으면 executing-plans보다 더 권장된다고 본다
