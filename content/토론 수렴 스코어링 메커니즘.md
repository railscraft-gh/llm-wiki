...
## 한 줄 정의
멀티 에이전트의 토론 [[워크플로]]에서 대립하는 에이전트들의 지엽적인 논박이나 평행선 논쟁을 방지하기 위해 중립적 사회자(Moderator)와 채점 에이전트(Scoring Agent)를 배치해 매 라운드 합치도 점수를 매기고 조기 종료(Early stopping)를 유도하는 구조화된 조율 기법이다.
...
## 출처
- raw/나만의 개인용 에이전트 시스템 개발하기: 단계별 가이드.md (Multi-Agent Discussion loop with early stopping)
