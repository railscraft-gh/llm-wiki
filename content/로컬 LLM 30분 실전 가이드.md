...
> [!summary]
> - 구독료 지출과 인터넷 유출 우려 없이 개인 기기(PC/Mac)에서 Ollama 엔진을 통해 고성능 로컬 모델을 구동하는 온디바이스 AI 가이드다.
> - VS Code 에디터(Cline 연동), 경량 벡터리스 RAG 스크립트, 실시간 로컬 오프라인 음성 비서 파이프라인의 3대 실무 구축 경로를 설명한다.
> - 로컬 구동 시 속도는 메모리 대역폭(Memory Bandwidth)에 직접 비례하므로, RAM/VRAM 가용 범위 내에서 모델 스펙을 영리하게 계층화해야 한다.
