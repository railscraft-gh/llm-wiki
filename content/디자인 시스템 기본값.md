...
- **[[단일 진실 공급원]]으로서의 [[디자인 토큰]] (Design Tokens)**: 색상 코드나 픽셀 값을 직접 입력하는 대신 `brand-blue`, `spacing-large`처럼 의미론적 이름을 붙여 한곳에서 중앙 관리한다.
...
2. **2단계: 폼(Forms)**: 입력창(Input), 라벨(Label), 에러 메시지 등 UX 마찰이 크고 불일치가 잦은 영역이다. 이미 버튼에서 확립한 [[간격 스케일]]과 반경 규칙을 조화롭게 대입한다.
...
- **빌드형 도구 vs 런타임 CSS**: [[Style Dictionary]](멀티 플랫폼을 위해 JSON 기반 토큰을 각각 내보내는 도구)나 Theo 같은 설정 도구는 다중 플랫폼 개발(iOS, Android, Web 동시 대응) 시 강력한 우위를 보이지만, 단일 웹 프로젝트나 1인 개발에는 환경 구축 비용과 복잡성 대비 효율이 떨어진다. 단일 웹 환경에서는 런타임 CSS 변수가 더 적합하다. 
  ...
  - **Theo**: Salesforce에서 개발한 도구로, [[Style Dictionary]]와 유사하지만 설정 문법이 간결함 (단, 2025년 기준 메인터넌스가 뜸함).
