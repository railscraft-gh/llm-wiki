...
- **도구 사용(Tool Use / Function Calling) 및 특정 모델 한계점**:
  ...
  - **Kimi (`moonshotai/kimi-k2.6`)**: 테스트 결과 도구 호출 자체는 수행하나, 한글 질문으로 대화를 이어가던 중 갑자기 중국어를 남발하며 횡설수설하는 시스템 언어 정렬(Alignment)의 명백한 한계(환각 현상)가 관찰된다.
  - **Gemma (`google/gemma-4-31b-it` 등)** 및 **GLM (`glm-5.1` 등)**: 모델의 기본 지능은 준수하나 무료 추론 레이어 상의 응답 속도(Latency)가 너무 느려, 실무적인 자율 코딩 에이전트 루프에 실시간 결합하여 사용하기에는 다소 무리가 있다.
