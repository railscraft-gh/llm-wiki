...
- 좋은 디자인 에이전트의 4대 속성은 명확성(intent alignment), 콘텍스트 인지(context awareness), 도구 오케스트레이션(tool orchestration), 설명 가능성(explainability)이다.
- [[Claude Code]] CLI에서 `/agents` 명령을 통해 `Running`과 `Library` 탭을 조회하고, `Library` 탭에서 신규 에이전트를 동적으로 설계 및 등록할 수
...
### 에이전트 생성 및 설정 가이드
- **명확성(Intent alignment)**: 단순 태스크 지시가 아닌, 성공과 실패의 엄밀한 기준을 명문화하여 모호한 문제를 가치 평가로 변환.
- **콘텍스트 인지(Context awareness)**: 현재의 대화 기록에만 머무는 것이 아니라, workflow의 각 단계 및 조직의 가이드라인([[DESIGN.md]] 등)을 적극 참조하는 형태.
- **도구 오케스트레이션(Tool orchestration)**: 적절한 MCP와 API 도구(Miro, FigJam, App Store Crawlers 등)를 과용 없이 순차 연동.
- **설명 가능성(Explainability)**: 판단의 이유, 관련 trade-off, 의사결정의 근거를 기록으로 표시.
...
### 에이전트 로컬 배포 경로 및 설정 예시
- [[Claude Code]] CLI에 `/agents` 명령어를 기입하여 생성할 수도 있지만, 아래 파일 시스템 경로에 직접 마크다운 포맷의 스키마 파일을 두어 배포할 수 있다.
```bash
# personal scope
mkdir -p ~/.claude/agents
cp research-synthesizer.md ~/.claude/agents/
