...
또한 [[LLM]] Wiki 패턴을 예시로 들었다. 매번 원시 문서에서 [[RAG]]로 답변하는 대신, 에이전트가 원시 소스를 점진적으로 지속적인 마크다운 위키로 편집하는 workflow다. 요약, 개체 페이지, 개념 페이지, 모순점, 교차 링크, 로그, 진화하는 종합을 유지한다. 이는 예전에는 불가능했던 새로운 정보 변환 형태다. 이 vault 관점의 운영 규칙은 [[LLM Wiki 운영 패턴]]으로 따로 분리해 볼 수 있다.
