...
- **OAuth 및 JWT 기반 미세 권한 통제 (Fine-grained AuthZ)**: 클라이언트에서 로그인한 최종 사용자의 JWT를 레지스트리 및 MCP 서버로 전달하고, 코드 수준에서 `@authorize_tool` 데코레이터를 이용해 도구 호출 권한을 부서/그룹 단위로 차단한다. [출처](../raw/Building%20an%20MCP%20Ecosystem%20at%20Pinterest-ko.md#L83)
