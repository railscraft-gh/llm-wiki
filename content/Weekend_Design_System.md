...
---
## 한 줄 정의
거창한 도구(Figma Tokens, Storybook 등) 없이 CSS 변수와 기초 컴포넌트(버튼, 폼, 카드)만으로 구축하여, 1인 개발자가 의사결정 피로를 줄이고 개발 속도를 높이는 경량화된 [[디자인 시스템]]입니다.
...
## 상세
사이드 프로젝트가 커질수록 발생하는 UI 일관성 붕괴와 매번 똑같은 디자인 결정을 반복하는 의사결정 피로를 해결하는 것이 핵심입니다. 복잡한 [[워크플로]]우를 먼저 도입하기보다, CSS 변수로 정의된 토큰을 프로젝트의 모든 컴포넌트가 공유하게 하여 구조를 잡는 것만으로도 디자인 생산성을 40% 이상 개선할 수 있습니다.
