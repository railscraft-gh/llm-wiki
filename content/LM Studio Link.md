...
- **분산 AI [[워크플로]]우**: 무거운 추론 작업은 거점 기기(Home server)가 담당하고, 사용자는 배터리를 소모하지 않으며 가벼운 기기로 인터페이스만 즐긴다. 
- **투명한 연결 방식**: `localhost:1234`의 OpenAI 호환 API 대신, `lmstudio.local:1234` 형태의 원격 엔드포인트를 제공하여 기존 애플리케이션 및 스크립트 [[워크플로]]우를 그대로 유지한다.
