...
## 한 줄 정의
[[LLM]]을 두뇌(핵심 컨트롤러)로 삼아 계획(Planning), 기억(Memory), 도구 사용(Tool use)을 결합해 복잡한 문제를 해결하는 자율 시스템.

## 핵심 요지
자율 에이전트는 [[LLM]]의 범용적인 문제 해결 능력을 극대화하기 위해 작업을 쪼개고 반성하는 Planning, 컨텍스트와 외부 지식을 저장/조회하는 Memory, 한계를 극복하기 위해 외부 API를 호출하는 Tool use의 세 가지 핵심 요소로 구성된다.
