...
- **맥락(Context) 기반의 미학 극복**: 디자인은 단순 미학이 아닌 **맥락**에서 시작되어야 하므로, 모델에 클라이언트의 PRD, 의견이 달린 PRD, 스타일가이드, 타깃 사용자 목표(Goal) 문서 등을 풍부히 공급한 상태에서 시작해야 하지. 그렇지 않으면 Framer 템플릿이나 뻔한 UI 조립('AI Slop')을 양산하여 제품의 실질 매출 및 사용자 신뢰를 오히려 해친다.
