...
- **최적화 단계의 진화**:
  ...
  - 하네스 자체 개선: [[Meta-Harness]], Self-Harness 등 [[LLM]] 코딩 에이전트가 실패 궤적을 분석해 자신의 제어 흐름과 도구 코드를 스스로 편집, 개선.

## 예시
- **AI Scientist 파이프라인**: 연구 아이디어 제안, 코드 작성, 실험 실행, 결과 분석, 논문 작성, 피어 리뷰에 이르는 전체 워크플로를 자동화한 시스템 설계.
- **Darwin Gödel Machine (DGM)**: 코딩 에이전트가 벤치마크 평가 로그를 분석하고, bash 셸과 편집기를 이용해 자신의 하네스 소스 코드를 수정한 뒤, 우수한 성과를 내는 하네스만 살아남는 진화적 탐색 과정.
