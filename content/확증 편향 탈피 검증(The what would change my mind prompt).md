...
- **[[LLM]] 기반 역발상 시나리오 검토**: [[LLM]]에게 역방향(Counter-factual) 반증 자료와 모순점을 짚어내도록 강제하여 컨텍스트 왜곡을 사전에 예방한다.
...
3. **반증 시나리오 도출**: [[LLM]]이 반대 의견의 관점, 숨겨진 리스크, 가설이 붕괴될 만한 치명타적 조건을 나열하도록 한다.
...
- [ ] [[LLM]]이 제시한 반증 시나리오가 현실적인 범위의 논리적 모순이나 리스크를 제대로 짚었는가?
- [ ] 반증 조건이 단순 추상이 아닌, 실제 프로젝트나 실무 환경에서 모니터링 가능한 구체적 지표(Metric)로 표현되었는가?

## 예시 시나리오

특정 피처 도입으로 고객 잔존율(Retention rate)이 15% 상승할 것이라 강하게 확신하는 기획 문서가 있다고 가정하자. 이 가설을 검증하기 위해 다음과 같은 [[워크플로]]를 수행한다.
