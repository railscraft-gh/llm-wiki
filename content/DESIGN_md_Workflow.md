...
---
## 한 줄 정의
`DESIGN.md`는 [[디자인 시스템]] 명세(색상, 서체, 컴포넌트 등)를 마크다운 파일로 관리하여, AI 에이전트(Stitch, [[Claude Code]] 등)가 일관된 UI를 구현하도록 돕는 차세대 디자인-코드 핸드오프 [[워크플로]]우입니다.
...
## 상세
`DESIGN.md` 기반 [[워크플로]]우는 기존의 번거로운 핸드오프 절차를 제거합니다. 디자이너는 Stitch 같은 도구로 시안을 만들고 기록된 마크다운을 코드 프로젝트 루트에 배치합니다. [[Claude Code]]와 같은 개발 에이전트는 이를 읽어 브랜드 일관성을 지키며 화면을 구현합니다. 이는 '[[디자인 시스템]]'이 복잡한 툴 없이 마크다운 한 장으로 관리되는 '경량화된 체계'로 발전했음을 보여줍니다.
