...
에러가 났을 때 당황하지 않고 해결하는 체계적인 접근법이다. (출처: CC101)
...
Claude Code 시작 전, 작업 중, 작업 후에 확인해야 할 항목들이다. (출처: CC101)
...
AI 모델이 실제로 존재하지 않는 패키지를 자신 있게 추천(hallucination)할 때, 공격자가 그 이름으로 악성 코드가 담긴 패키지를 npm/PyPI에 미리 올려두는 공격 기법이다. (출처: CC101)
...
Claude가 변경한 모든 것을 한꺼번에 스테이징하는 대신, 변경 덩어리(hunk)마다 포함 여부를 선택할 수 있다. (출처: CC101)
...
**원칙**: Claude가 생성한 코드도 외부에서 받은 코드처럼 검토하세요. AI가 작성했다고 안전이 보장되지 않는다. (출처: CC101)
