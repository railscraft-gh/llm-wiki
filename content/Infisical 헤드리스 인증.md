...
- SSH 서버 환경에서는 `infisical login`을 대신하여 Machine Identity + Universal Auth 방식으로 환경 변수를 주입하는 것이 보안상 가장 안전하다. (출처: raw/[[Infisical]]_SSH_헤드리스_인증_가이드.md)
- 서버의 인증 파일은 root 소유로 두고 chmod 600 권한 제한을 강제하여 외부 접근을 차단한다. (출처: raw/[[Infisical]]_SSH_헤드리스_인증_가이드.md)
